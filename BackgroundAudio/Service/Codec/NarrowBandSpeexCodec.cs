...
namespace BackgroundAudio.Service.Codec
{
    ...
    abstract class SpeexChatCodec : INetworkChatCodec
    {
        ...
        public byte[] Encode(byte[] data, int offset, int length)
        {
            ...
            //Console.WriteLine(String.Format("NSpeex: In {0} bytes, encoded {1} bytes [enc frame size = {2}]", length, bytesWritten, encoder.FrameSize));
            return encoded;
        }
        ...
        public byte[] Decode(byte[] data, int offset, int length)
        {
            ...
            //Console.WriteLine(String.Format("NSpeex: In {0} bytes, decoded {1} bytes [dec frame size = {2}]", length, bytesDecoded, decoder.FrameSize));
            return decoded;
        }
